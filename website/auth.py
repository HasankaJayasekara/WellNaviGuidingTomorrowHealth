    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                ...
                return redirect(url_for('views.predict'))  # Redirect to regular  page
            else:
...
def admin_sign_up():
    if request.method == 'POST':
        ...
        admin = Admin.query.filter_by(email=email).first()
        if admin:
            flash('Email already exists for an admin.', category='error')
        elif len(email) < 4:
            flash('Email must be greater than 3 characters.', category='error')
        elif len(name) < 2:
            ...
            new_admin = Admin(email=email, name=name, password=generate_password_hash(
                password1, method='pbkdf2:sha256'))
